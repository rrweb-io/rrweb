...
Create `scripts/check-plugin-host-smoke.mjs` with these acceptance criteria:

- assert every required package has a built `dist` directory before packing;
- scan every plugin package's built `.d.ts` and `.d.cts` files and fail if a plugin declaration imports `rrweb`, `rrdom`, or `rrweb-snapshot`;
- pack `@rrweb/types`, `@rrweb/utils`, `rrdom`, `rrweb-snapshot`, `rrweb`, `@rrweb/record`, `@rrweb/replay`, and all eight plugin packages with `npm pack --json --ignore-scripts`;
- create a strict throwaway `@rrweb/record` consumer that imports `@rrweb/record` plus the console, network, sequential-id, and canvas WebRTC record plugins;
- create a strict throwaway `@rrweb/replay` consumer that imports `@rrweb/replay` plus the console, network, sequential-id, and canvas WebRTC replay plugins and their paired record plugin dependencies;
- install each consumer with `npm install --ignore-scripts` and compile it with `npx tsc --noEmit`;
- use `skipLibCheck: true` in the throwaway consumers so existing host-package declaration internals do not hide the plugin entrypoint signal;
