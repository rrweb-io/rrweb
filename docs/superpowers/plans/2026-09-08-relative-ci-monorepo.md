...
**Architecture:** The shared Vite configuration emits one webpack-compatible stats file for each tracked package when CI enables `RELATIVE_CI_STATS`. The unprivileged ESLint workflow uploads package-specific RelativeCI artifacts. A successful `workflow_run` starts a least-privilege matrix job that sends each artifact to its matching RelativeCI project with one package-scoped secret.

**Tech Stack:** GitHub Actions, Vite 6, `rollup-plugin-webpack-stats` 3.1.3, `relative-ci/agent-upload-artifact-action` v2, `relative-ci/agent-action` v3, Yarn 1
...
```bash
yarn add --dev --exact rollup-plugin-webpack-stats@3.1.3
```
...
Import the plugin and extend the existing options type with `bundleStats?: boolean`. Default that option to `true`. Append this as the final plugin entry when `RELATIVE_CI_STATS === 'true'` and `bundleStats` is enabled:

```ts
...(process.env.RELATIVE_CI_STATS === 'true' && bundleStats
  ? [webpackStatsPlugin({ excludeAssets: /\.map$/ })]
  : []),
```

This keeps local builds unchanged and matches the old collector's exclusion of source maps.

- [ ] **Step 3: Preserve the packer exclusion**
...
- [ ] **Step 4: Build one representative package with stats enabled**
...
Expected: Vite exits 0 and `jq` exits 0 for the generated webpack-compatible stats object.
...
```bash
yarn prettier --check package.json vite.config.default.ts packages/packer/vite.config.ts
yarn tsc --noEmit --skipLibCheck --moduleResolution node --module esnext --target es2020 vite.config.default.ts
```

Expected: both commands exit 0.

### Task 2: Upload RelativeCI artifacts from the unprivileged build
...
- [ ] **Step 3: Upload one RelativeCI artifact per tracked package**

After linting, add one pinned `relative-ci/agent-upload-artifact-action` step per package. Every step uses commit `a2b5741b4f7e6a989c84ec1a3059696b23c152e5`, a unique `artifactName`, and the package's `dist/webpack-stats.json`. Include the 11 top-level packages and eight plugins listed in the design. Do not upload a packer artifact.

Example entry:

```yaml
- name: Upload @rrweb/record bundle stats
  uses: relative-ci/agent-upload-artifact-action@a2b5741b4f7e6a989c84ec1a3059696b23c152e5 # v2
  with:
    artifactName: relative-ci-record
    webpackStatsFile: packages/record/dist/webpack-stats.json
```
...
rg -c 'agent-upload-artifact-action@' .github/workflows/eslint-check.yml
```

Expected: formatting passes and the count is `19`.
...
Use a matrix with one entry per artifact. Each entry contains `artifact`, `name`, and `secret`. Map secret names consistently, for example:

```yaml
- artifact: relative-ci-record
  name: '@rrweb/record'
...
The matrix job contains no checkout or shell step. Send each artifact using commit `fcf45416581928e8dd62eded78ce98c78e5149f8`:
...
rg -c '^\s+- artifact: relative-ci-' .github/workflows/pr-checks-privileged.yml
```
...
Expected: both files parse without errors and retain a top-level `on` key.
...
```bash
yarn prettier --check package.json vite.config.default.ts packages/packer/vite.config.ts .github/workflows/eslint-check.yml .github/workflows/pr-checks-privileged.yml docs/superpowers/specs/2026-09-08-relative-ci-monorepo-design.md docs/superpowers/plans/2026-09-08-relative-ci-monorepo.md
```
...
Confirm that the PR workflow references no `RELATIVE_CI_KEY`, the privileged RelativeCI job has no checkout or `run` step, and new third-party actions use full commit SHAs.
