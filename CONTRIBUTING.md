...
- Build all packages: (in `/`) `pnpm run build:all` or `pnpm run dev`
- Run recorder on a website: (in `/packages/rrweb`) `pnpm run repl`
- Run a cobrowsing/mirroring session locally: (in `/packages/rrweb`) `pnpm run live-stream`
- Build individual packages: `pnpm run build` or `pnpm run dev`
- Test: `pnpm test` or `pnpm run test:watch`
- Lint: `pnpm run lint`
- Rewrite files with prettier: `pnpm run format` or `pnpm run format:head`
