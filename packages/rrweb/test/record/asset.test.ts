...
describe('asset capturing', function (this: ISuite) {
  ...
  describe('captureAssets partial defaults', function (this: ISuite) {
    const ctx: ISuite = setup.call(
      this,
      `
        <!DOCTYPE html>
    ...
    it('keeps default objectURLs true when only stylesheets is configured', async () => {
      await waitForRAF(ctx.page);
      const url = (await ctx.page.evaluate(() => {
        return document.querySelector('img')?.src;
      })) as string;
...
`,
  ...
  describe('full snapshot stylesheet asset timestamps', () => {
    const ctx: ISuite = setup.call(
      this,
      `
<!DOCTYPE html>
...
<head>
  ...
  <style>.sync-capture { color: red; }</style>
</head>
</html>
`,
      {
    ...
    it('uses the full snapshot timestamp for synchronous stylesheet assets', async () => {
      await waitForRAF(ctx.page);
      ...
      const assetEvent = events.find(
        (e) =>
          ...
          e.data.payload?.rr_type === 'CssText' &&
          e.data.payload.cssTexts.some((cssText) =>
            cssText.includes('.sync-capture'),
          ),
      );
      ...
      expect(assetEvent?.timestamp).not.toBe(0);
    });
  });

  describe('full snapshot asset failure timestamps', () => {
    const ctx: ISuite = setup.call(
      this,
      `
<!DOCTYPE html>
...
<body>
  <img src="failprotocol://example.com/full-snapshot-image.png" />
</body>
</html>
`,
      {
    ...
    it('uses the full snapshot timestamp for full snapshot asset failures', async () => {
      await ctx.page.waitForTimeout(500);
      await waitForRAF(ctx.page);
      ...
      const url = 'failprotocol://example.com/full-snapshot-image.png';
      const fullSnapshotEvent = events.find(
        (e) => e.type === EventType.FullSnapshot,
      );
      const failureEvent = events.find(
        (e) => e.type === EventType.Asset && e.data.url === url,
      );

      expect(fullSnapshotEvent).toMatchObject({
        data: {
          capturedAssetStatuses: expect.arrayContaining([
            {
              url,
              status: 'capturing',
            },
