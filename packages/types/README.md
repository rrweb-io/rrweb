...
This package contains the shared types used across rrweb packages.  rrweb makes extensive use of typescript types to enforce correctness and define the data API between packages.  Check out the [Dive Into Events](./docs/recipes/dive-into-event.md) recipe for an introduction to the principal `Event` object. [View the source code](https://github.com/rrweb-io/rrweb/blob/main/packages/types/src/index.ts) to get a flavour the types available, or check out the [Contributor Guide](../../CONTRIBUTING.md) for more info.
