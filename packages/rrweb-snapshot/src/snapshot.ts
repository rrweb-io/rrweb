...
export function classMatchesRegex(
  ...
  if (!checkAncestors) return -1;
  return classMatchesRegex(node.parentNode, regex, checkAncestors, distance + 1);
}
...
export function needMaskingText(
  ...
  let mask = !!maskAllText;
  let maskDistance = -1;
  let unmaskDistance = -1;

  if (mask) {
    if (typeof unmaskTextClass === 'string') {
      unmaskDistance = closestMatch(el, `.${unmaskTextClass}`);
      if (unmaskDistance >= 0) {
        mask = false;
      }
    } else {
      unmaskDistance = classMatchesRegex(el, unmaskTextClass, true);
      if (unmaskDistance >= 0) mask = false;
    }
  }

  if (mask && unmaskTextSelector) {
    unmaskDistance = closestMatch(el, unmaskTextSelector);
    if (unmaskDistance >= 0) {
      mask = false;
    }
  }
  ...
  if (!mask) {
    if (typeof maskTextClass === 'string') {
      maskDistance = closestMatch(el, `.${maskTextClass}`);
      if (maskDistance >= 0 && (unmaskDistance < 0 || maskDistance < unmaskDistance)) {
        mask = true;
      }
    } else {
      maskDistance = classMatchesRegex(el, maskTextClass, true);
      if (maskDistance >= 0 && (unmaskDistance < 0 || maskDistance < unmaskDistance)) mask = true;
    }
  }

  if (!mask && maskTextSelector) {
    maskDistance = closestMatch(el, maskTextSelector);
    if (maskDistance >= 0 && (unmaskDistance < 0 || maskDistance < unmaskDistance)) {
      mask = true;
    }
  }
...
function serializeTextNode(
  ...
  const { maskAllText, maskTextClass, unmaskTextClass, maskTextSelector, unmaskTextSelector, maskTextFn, rootId } = options;
  ...
  if (
    ...
    !isScript &&
    textContent &&
    needMaskingText(n, maskTextClass, maskTextSelector, unmaskTextClass, unmaskTextSelector, maskAllText)
  ) {
