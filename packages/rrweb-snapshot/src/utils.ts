...
export function getUntaintedChildNodesAccessor(): (
  ...
  try {
    ...
    // const untaintedNode = win.Node
    // Create an element using the iframe's document
    const untaintedElement = win.document.createElement('div');
    // Access the Element constructor
    const ElementConstructor = untaintedElement.constructor;
...
export function childNodes(n: Node): NodeListOf<Node> {
  ...
  if (useOriginal) n.childNodes;

  return untaintedChildNodesAccessor.call(n) as NodeListOf<Node>;
}
